...
// a "good" control string would be aaabbbbhaijjjm meaning that the printer
// used three times color a, four times color b, then one time color a...
//
...
function printerError(s) {
  var colorLetters = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm'];
  var totalLetters = s.length;
  var errorLetters = 0;
  for (var i=0; i<s.length; i++) {
    if (!colorLetters.includes(s[i])) {
      errorLetters++;
    }
  }
  return errorLetters.toString() + '/' + totalLetters.toString();
}
